...
Создал каталог task5 и два файла compose.yaml и docker-compose.yaml в ней, после чего выполнил команду  
```bash
docker compose up -d
```
...
После удаления файла compose.yaml и выполнения команды "" получаем предупреждение, Docker Compose обнаружил "сиротские контейнеры", конкретно task5-portainer-1, так как в docker-compose.yaml файле больше не упоминается данный сервис, который ранее был запущен. Эти контейнеры могут быть удалены с помощью флага --remove-orphans.
